...
class PageNavigator(
    ...
    private fun onGateEntry(state: TrackingState) {
        ...
        if (autoOpenEnabled) {
            // One plain ShowMapPage with the library-default zoom, nothing else.
            // The previous choreography — zoom=false plus up to eight ZoomPage
            // presses at 300ms intervals — threw the ride app to the Karoo home
            // screen on entry; the default call is the path other shipped
            // extensions exercise.
